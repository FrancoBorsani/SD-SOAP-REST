...
with app.app_context():
    ...
    class User( db.Model):
        ...
        def __repr__(self):
            rep = '(' '['+ self.username +','+ self.password + ','+ self.nombre +',' +self.apellido+','+str(self.dni)+','+ self.email +']' ')'
            return rep
    ...
    class Sales(CorsService):
        ...
        def get_users(id):
            product = User.query.filter_by(id=id)
        ...
        def register(nombre, apellido,username,password,dni,email,telefono):
            ...
            new_user = User(username=username, password=password, nombre=nombre, apellido=apellido,dni=dni, email=email,telefono=telefono)
    ...
    if __name__=='__main__':
        ...
        application = Application([Sales], 'spyne.examples.hello.http',
            ...
            out_protocol=JsonDocument(complex_as=True),
        )
